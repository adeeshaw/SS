...
# internal flags
NVCCFLAGS   := -maxrregcount=32 -m${TARGET_SIZE}
CCFLAGS     :=
LDFLAGS     :=
...
LIBRARIES += -L$(CUDA_PATH)/lib$(LIBSIZE) -lcufft -lcudart
...
#	$(EXEC) $(MPICXX) -fopenmp $(INCLUDES) $(MPI_CCFLAGS) -o $@ -c $<

#simpleMPI.o:simpleMPI.cu
#	$(EXEC) $(NVCC) $(INCLUDES) $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<

#simpleMPI: simpleMPI_mpi.o simpleMPI.o
#	$(EXEC) $(MPICXX) -fopenmp $(MPI_LDFLAGS) -o $@ $+ $(LIBRARIES)
#	$(EXEC) mkdir -p ../../bin/$(TARGET_ARCH)/$(TARGET_OS)/$(BUILD_TYPE)
#	$(EXEC) cp $@ ../../bin/$(TARGET_ARCH)/$(TARGET_OS)/$(BUILD_TYPE)

simpleMPI_mpi.o:simpleMPI.cpp 
	$(EXEC) icc -O3  -IPF-fp-relaxed -no-prec-div -no-prec-sqrt -Ofast -fp-model fast=2  -align   -march=corei7-avx  -fopenmp $(INCLUDES) $(MPI_CCFLAGS) -o $@ -c $<

simpleMPI.o:simpleMPI.cu
	$(EXEC) $(NVCC) $(INCLUDES) $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<

simpleMPI: simpleMPI_mpi.o simpleMPI.o
	$(EXEC)  icc -O3  -IPF-fp-relaxed -no-prec-div -no-prec-sqrt -Ofast -fp-model fast=2  -align   -march=corei7-avx  -fopenmp $(MPI_LDFLAGS) -o $@ $+ $(LIBRARIES)
	$(EXEC) mkdir -p ../../bin/$(TARGET_ARCH)/$(TARGET_OS)/$(BUILD_TYPE)
	$(EXEC) cp $@ ../../bin/$(TARGET_ARCH)/$(TARGET_OS)/$(BUILD_TYPE)
